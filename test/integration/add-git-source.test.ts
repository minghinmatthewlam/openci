...
describe('integration: add git source', () => {
  it('installs from a cloneable git source with source-first semantics', () => {
    ...
    execFileSync('git', ['config', 'user.name', 'OpenCI Test'], { cwd: targetRepo, stdio: 'ignore' });
    execFileSync('git', ['config', 'user.email', 'test@example.com'], { cwd: targetRepo, stdio: 'ignore' });
    execFileSync('git', ['commit', '--allow-empty', '-m', 'init'], { cwd: targetRepo, stdio: 'ignore' });
    execFileSync('git', ['remote', 'add', 'origin', 'https://github.com/acme/project.git'], { cwd: targetRepo, stdio: 'ignore' });
